...
`fortserver-WebLite-5.0.0-x64.msi`，不再解压浏览器 ZIP。
MSI 将完整运行时直接安装到 `%ProgramFiles%\fortserver\WebLite`，供发布机上的所有用户使用。
安装包需同步到 Core 下载服务的 `/download/applets/`，以便通过 `jms:///` 下载。
上游安装包：[Luna v5.0.0](https://github.com/fortserver/luna/releases/download/v5.0.0/fortserver-WebLite-5.0.0-x64.msi)。
