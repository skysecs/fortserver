...
- The packages are served by the fortserver download site, the applet host only needs to reach fortserver itself and requires no internet access
...
- The interface language follows the user's language setting in fortserver regardless of the applet host Windows language, and falls back to English when DBX has no matching locale
