...
class WebAppletDefaultsTests(SimpleTestCase):
    def test_weblite_msi_package_version_and_stable_executable_path(self):
        ...
        installer = f'fortserver-WebLite-{version}-x64.msi'
        ...
        self.assertEqual(
            manifest['path'],
            '%ProgramFiles%/fortserver/WebLite/weblite.exe',
        )
...
class DamengConnectMethodTests(SimpleTestCase):
    @override_settings(VENDOR='fortserver')
...
class AppProviderDeploymentTests(SimpleTestCase):
    ...
    def test_generated_playbook_binds_panda_to_provider(self):
        provider = mock.Mock(
            ...
            host=mock.Mock(address='192.0.2.10'),
            deploy_options={
                'PANDA_IMAGE': 'fortserver/panda:test',
        ...
        self.assertEqual(variables['PANDA_IMAGE'], 'fortserver/panda:test')
...
class AppProviderDeploymentAPITests(TestCase):
    def setUp(self):
        ...
        serializer = AppProviderSerializer(data={
            ...
            'deploy_options': {
                'CORE_HOST': 'https://core.example.com', 'PANDA_IMAGE': 'fortserver/panda:test',
            },
        })
        serializer.is_valid(raise_exception=True)
    ...
    def test_missing_bundle_and_unspecified_panda_image_use_core_version(self):
        ...
        with tempfile.TemporaryDirectory() as data_dir, override_settings(DATA_DIR=data_dir, VERSION='dev'):
            self.assertEqual(self.provider.validate_deployment()['PANDA_IMAGE'], 'fortserver/panda:dev')
...
class OfflineProviderResourcesTests(SimpleTestCase):
    def setUp(self):
        ...
        self.image = 'fortserver/panda:v5.0-ee'
        self.metadata = {
    ...
    def test_missing_bundle_uses_core_version(self):
        self.assertEqual(default_panda_image(), 'fortserver/panda:v5.0.1-ee')
    ...
    def test_options_default_and_cleared_override_use_exact_bundle_image(self):
        ...
        self.assertEqual(metadata.get_field_info(field).get('default'), 'fortserver/panda:v5.0.1-ee')
        self.write_manifest()
