...
def _signing_key(key_id):
    return get_signing_key(
        ...
        purpose='fortserver-chat-ai-delegation-v1',
    )
...
def issue_delegation(
    *, user_id, org_id, conversation_id, approval_id, operation_id, method,
    ...
    payload = {
        'issuer': getattr(settings, 'CHAT_AI_DELEGATION_ISSUER', 'fortserver-ai'),
        'audience': getattr(settings, 'CHAT_AI_DELEGATION_AUDIENCE', 'fortserver-core'),
        'key_id': key_id,
        'user_id': str(user_id),
        'org_id': str(org_id),
...
def verify_delegation(token):
    ...
    if not all(payload.get(key) for key in required):
        return None
    if payload['issuer'] != getattr(settings, 'CHAT_AI_DELEGATION_ISSUER', 'fortserver-ai'):
        return None
    if payload['audience'] != getattr(settings, 'CHAT_AI_DELEGATION_AUDIENCE', 'fortserver-core'):
        return None
    return payload
