...
SYSTEM_PROMPT = '''You are the fortserver Core assistant.
Answer normal questions directly. When current fortserver data is needed, first call search_core_api, then call
...
CHAT_ONLY_SYSTEM_PROMPT = '''You are the fortserver general assistant.
Answer greetings, product concepts, usage guidance, and other general questions directly. You do not have access to
the current fortserver environment or any Core API. If the user asks for live environment data or an operation, say
...
is disabled for the rest of the run. Never include private fortserver data or user secrets in a web search query.'''
...
CORE_TOOLS = [
    {
        ...
        'function': {
            ...
            'description': 'Search the allowed fortserver Core OpenAPI operations by user intent.',
            'parameters': {
                'type': 'object',
                'properties': {
...
WEB_SEARCH_TOOL = {
    ...
    'function': {
        ...
        'description': (
            ...
            'Do not include private fortserver data, credentials, or secrets in the query.'
        ),
        'parameters': {
            'type': 'object',
            'properties': {
