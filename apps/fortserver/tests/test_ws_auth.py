...
from fortserver.routing import get_signature_user
...
class WebSocketAuthenticationTests(SimpleTestCase):
    ...
    def test_oauth_bearer_authenticates_face_websocket(self):
        ...
        with patch('fortserver.routing.SignatureAuthentication.authenticate', return_value=None), \
             patch('fortserver.routing.AccessTokenAuthentication.authenticate', return_value=None), \
             patch('fortserver.routing.OAuth2Authentication.authenticate', return_value=(user, object())) as oauth:
    ...
    def test_missing_authorization_does_not_try_oauth(self):
        with patch('fortserver.routing.OAuth2Authentication.authenticate') as oauth:
    ...
    def test_invalid_bearer_does_not_authenticate(self):
        with patch('fortserver.routing.SignatureAuthentication.authenticate', return_value=None), \
             patch('fortserver.routing.AccessTokenAuthentication.authenticate', return_value=None), \
             patch('fortserver.routing.OAuth2Authentication.authenticate', return_value=None):
