...
- fortserver 运行版本来自 `apps/fortserver/const.py`，当前为 `2.0.0`；`pyproject.toml` 的包元数据版本为 `v4.0`，两者当前不一致，镜像构建会重写 `const.VERSION`。
...
- Core API 在 `fortserver.urls.resource_api` 下统一挂载到 `/api/v1/`。
...
- Chat AI Loader 直接以 `fortserver.urls` 调用内部 Schema Generator，不解析 Swagger HTML；结果按 TTL 缓存并生成 SHA-256 Schema Hash。
- Core WSGI 为 `fortserver.wsgi:application`；现有 ASGI 为 `fortserver.asgi:application`，HTTP 最终进入 Channels 的 Django ASGI Application。
...
后台消息和定时报告“立即运行”共享用户级接口频率、待执行数量和每日 Token 配额；立即运行会在入队事务内创建 Run，因此连续点击不会产生重复队列项。定时报告复用 fortserver 的 Celery Beat `interval`/`crontab` 调度，注册名称使用完整 UUID；每次执行创建独立 Conversation，并在模型真正选定时记录实际模型，避免不同周期的上下文互相污染或历史模型为空。定时报告强制 `read_only=True` 且公网搜索默认关闭：即使全局白名单中存在写 operationId，也不会搜索或执行写操作，不会产生自动审批或无人值守变更。每个用户默认最多创建 10 个定时报告，由 `CHAT_AI_MAX_SCHEDULES_PER_USER` 控制。
...
```bash
cd apps
gunicorn fortserver.ai_asgi:application \
...
CHAT_AI_CORE_CA_CERT: /etc/fortserver/pki/ca.crt
CHAT_AI_CORE_CLIENT_CERT: /etc/fortserver/pki/chat-ai.crt
CHAT_AI_CORE_CLIENT_KEY: /etc/fortserver/pki/chat-ai.key
...
- `apps/fortserver/ai_asgi.py`、`ai_urls.py`、`settings/ai.py`：独立 AI ASGI 服务。
